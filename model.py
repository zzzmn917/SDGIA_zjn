...
class CombineGraph(Module):
    ...
    def compute_scores(self, hidden, mask):
        ...
        hs = torch.sum(hidden * mask, -2) / torch.sum(mask, 1)  # 每个样本的隐藏状态的加权平均值，其中权重由掩码确定。论文公式12的结果
        ...
        ns = torch.sigmoid(self.glu1(nh) + self.glu2(hs))  # 论文公式13除去权重矩阵q的结果
        beta = torch.matmul(ns, self.w_2)    # 论文公式13的结果，得到注意力权重
        beta = beta * mask   # 将注意力权重与掩码相乘，以过滤掉不相关的部分
        select = torch.sum(beta * hidden, 1)  # 论文公式14的结果

        # q1 = self.linear_one(ht.mean(dim=1))  # 对 ht 进行池化操作，例如使用平均池化
        # q2 = self.linear_two(hidden)  # batch_size x seq_length x latent_size
        # # 对整个序列的隐藏状态 hidden 进行线性变换
        #
        # # 得到注意力权重
        # alpha = self.linear_three(torch.sigmoid(q1.unsqueeze(1)+ q2))
